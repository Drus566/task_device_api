...
# Decision
Передавать функцию как параметр для выполнения в контексте довольно странно, в случае fastapi обычно создают пул до базы при старте приложения  (`@app.on_event("startup")`), и при завершении работы соединение закрывают. Само соединение можно сохранить в app.state, а получать потом через `request.app.state`. Альтернатива  сохранению в state - механизм зависимостей Depends. Контекст соединения лучше писать в каждой отдельной функции, где выполняется  SQL запрос - так удобнее управлять механизмами транзакций.  С моей точки зрения правильный вариант - написать функцию `add_devices`, в нее передать заранее созданный пул, а уже там создать коннект до базы
